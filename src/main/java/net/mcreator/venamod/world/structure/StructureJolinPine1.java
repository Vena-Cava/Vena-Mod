...
package net.mcreator.venamod.world.structure;
...
public class StructureJolinPine1 extends ElementsVenaMod.ModElement {
	public StructureJolinPine1(ElementsVenaMod instance) {
		super(instance, 86);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (dimID == WorldJolin.DIMID) {
			...
			isNetherType = WorldJolin.NETHER_TYPE;
		}
		...
		if ((random.nextInt(1000000) + 1) <= 1000000) {
			...
			for (int a = 0; a < count; a++) {
				...
				if (Biome.REGISTRY.getNameForObject(biome).equals(new ResourceLocation("venamod:jolinpineforest")))
					biomeCriteria = true;
				if (!biomeCriteria)
					continue;
				if (world.isRemote)
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("venamod", "pine_sapling_1"));
				if (template == null)
					return;
