...
import net.mcreator.venamod.block.BlockMangrovewood;
import net.mcreator.venamod.block.BlockMangroveleaves;
import net.mcreator.venamod.ElementsVenaMod;

import java.util.Random;

@ElementsVenaMod.ModElement.Tag
public class BiomeDarkSwamp extends ElementsVenaMod.ModElement {
	@GameRegistry.ObjectHolder("venamod:darkswamp")
	public static final BiomeGenCustom biome = null;
	public BiomeDarkSwamp(ElementsVenaMod instance) {
		super(instance, 62);
	}
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Dark Swamp").setRainfall(0.5F).setBaseHeight(-0.2F).setWaterColor(-16757170).setHeightVariation(0F)
					.setTemperature(0.8F));
			setRegistryName("darkswamp");
	...
	static class CustomTree extends WorldGenAbstractTree {
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getBlock().isLeaves(state, world, blockpos)
												...
												|| state.getBlock() == BlockMangroveleaves.block.getDefaultState().getBlock()) {
											this.setBlockAndNotifyAdequately(world, blockpos, BlockMangroveleaves.block.getDefaultState());
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							if (state.getBlock().isAir(state, world, genhPos) || state.getBlock() == Blocks.VINE.getDefaultState().getBlock()
									|| state.getBlock() == BlockMangroveleaves.block.getDefaultState().getBlock()) {
								this.setBlockAndNotifyAdequately(world, position.up(genh), BlockMangrovewood.block.getDefaultState());
								if (genh > 0) {
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == BlockMangrovewood.block.getDefaultState().getBlock()
					|| blockType == BlockMangroveleaves.block.getDefaultState().getBlock() || blockType == Blocks.GRASS.getDefaultState().getBlock()
