...
import net.mcreator.venamod.procedure.ProcedureCattaildrophead;
import net.mcreator.venamod.item.ItemCattailstalk;
import net.mcreator.venamod.ElementsVenaMod;

import java.util.Random;

@ElementsVenaMod.ModElement.Tag
public class BlockCattail extends ElementsVenaMod.ModElement {
	@GameRegistry.ObjectHolder("venamod:cattail")
	public static final Block block = null;
	public BlockCattail(ElementsVenaMod instance) {
		super(instance, 10);
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0, new ModelResourceLocation("venamod:cattail", "inventory"));
	}
	...
	public void generateWorld(Random random, int chunkX, int chunkZ, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (Biome.REGISTRY.getNameForObject(biome).equals(new ResourceLocation("venamod:wetsavanna")))
			biomeCriteria = true;
		if (Biome.REGISTRY.getNameForObject(biome).equals(new ResourceLocation("venamod:darkswamp")))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
		for (int i = 0; i < 40; i++) {
	...
	public static class BlockCustomFlower extends BlockReed {
		...
		public void getDrops(NonNullList<ItemStack> drops, IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
			drops.add(new ItemStack(ItemCattailstalk.block, (int) (1)));
		}
		...
		public boolean removedByPlayer(IBlockState state, World world, BlockPos pos, EntityPlayer entity, boolean willHarvest) {
			...
			int z = pos.getZ();
			{
				...
				ProcedureCattaildrophead.executeProcedure($_dependencies);
			}
			return retval;
		}
	}
