...
public class MCreatorDarkSwamp extends Elementsvenamod.ModElement {
	@GameRegistry.ObjectHolder("venamod:darkswamp")
	public static final BiomeGenCustom biome = null;

	public MCreatorDarkSwamp(Elementsvenamod instance) {
		super(instance, 44);
	}
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Dark Swamp").setRainfall(0.5F).setBaseHeight(-0.5F).setWaterColor(-16757170).setHeightVariation(0F)
					.setTemperature(0.8F));
	...
	static class CustomTree extends WorldGenAbstractTree {
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getBlock().isLeaves(state, world, blockpos)
												...
												|| state.getBlock() == MCreatorMangroveleaves.block.getDefaultState().getBlock()) {
											this.setBlockAndNotifyAdequately(world, blockpos, MCreatorMangroveleaves.block.getDefaultState());
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							if (state.getBlock().isAir(state, world, genhPos) || state.getBlock() == Blocks.VINE.getDefaultState().getBlock()
									|| state.getBlock() == MCreatorMangroveleaves.block.getDefaultState().getBlock()) {
								this.setBlockAndNotifyAdequately(world, position.up(genh), MCreatorMangrovewood.block.getDefaultState());
								if (genh > 0) {
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == MCreatorMangrovewood.block.getDefaultState().getBlock()
					|| blockType == MCreatorMangroveleaves.block.getDefaultState().getBlock()
